    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && layoutManager.getItemCount() > layoutManager.getChildCount() *///满一屏加载
                    && computeVerticalScrollExtent() + computeVerticalScrollOffset()
                        >= computeVerticalScrollRange() //不管满不满一屏都加载
                    && !isNoMore && mRefreshHeader.getState() < ArrowRefreshHeader.STATE_REFRESHING) {
                isLoadingData = true;
                Log.e("SCROLL_STATE_IDLE","state0");
                if (mFootView instanceof LoadingMoreFooter) {
       ...
       /* if (state==RecyclerView.SCROLL_STATE_IDLE&&!loadingMoreEnabled&&!isLoadingData){
            LayoutManager layoutManager = getLayoutManager();
            Log.e("SCROLL_STATE_IDLE","state1");
            if (layoutManager.getChildCount() > 0
                    && computeVerticalScrollExtent() + computeVerticalScrollOffset()
                    >= computeVerticalScrollRange() //不管满不满一屏都加载
                    && isNoMore && mRefreshHeader.getState() < ArrowRefreshHeader.STATE_REFRESHING){
                Log.e("SCROLL_STATE_IDLE","state2");
                if (mFootView instanceof LoadingMoreFooter){
                    Log.e("SCROLL_STATE_IDLE","state3");
                    ((LoadingMoreFooter) mFootView).setState(LoadingMoreFooter.STATE_LOADING);
                }
            }
        }*/
