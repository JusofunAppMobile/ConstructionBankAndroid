...
/**
 * @author zhaoyapeng
 * @version create time:17/12/2009:09
 * @Email zyp@jusfoun.com
 * @Description ${图形相关的计算 方法}
 */
public class MathHelper {
    ...
    public PointF getPointF(float angle, float X, float Y, float radius) {
        ...
        if (Float.compare(arcAngle, 0.0f) < 0) {
            ...
            mPointF.x = (float) (X + Math.cos(arcAngle) * radius);
            mPointF.y = (float) (Y + Math.sin(arcAngle) * radius);
     ...
     *  保留6位小数
     * */
    public static double doubleKeepDecimals(double num){
        //使用0.00不足位补0，#.##仅保留有效位
        double f1 = 0;
        try {
            BigDecimal bg = new BigDecimal(num);
             f1 = bg.setScale(6, BigDecimal.ROUND_DOWN).doubleValue();
        }catch (Exception e){
        }
        return f1;
    }
